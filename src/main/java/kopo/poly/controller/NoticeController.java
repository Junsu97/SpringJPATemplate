...
/**
 * Cotroller 선언해야만 Spring 프레임워크에서 Ctroller인지 인식가능
 * 자바 서블릿 역할 수행
 *
 * slf4j는 스프링 프레임워크에서 로그 처리하는 인터페이스 기술이며,
 * 로그처리 기술인 log4j와 logback과 인터페이스 역할 수행
...
public class NoticeController {
    ...
    /**
     * 게시글 등록
     * 게시글 등록은 Ajax로 호출되기 때문에 결과는 JSON 구조로 전달해야 한다.
     * JSON 구조로 결과 메시지를 전숭하기 위해 @ResponseBody 어노테이션 추가
     */
    @ResponseBody
    ...
    public MsgDTO noticeInsert(HttpServletRequest request, HttpSession session){
        ...
        try{
            // 로그인된 사용자 아이디 가져오기
            // 로그인을 아직 구현하지 않았기에 공지사항 리스트에서 로그인 한 것처럼 Session 값을 저장
            ...
            log.info("seesion user_id : " + userId);
    ...
    public MsgDTO noticeUpdate(HttpServletRequest request, HttpSession session) {
